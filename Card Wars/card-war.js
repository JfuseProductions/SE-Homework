...
function playGame(e) { // event object as a parameter in function play game
  let temp = e.target.textContent; //figure out what button got clicked  e.target.innerText or .innerHTML (IN THIS CASE IT WILL BE THE START BUTTON)
  //console.log(temp);
...
dealCards = playerCard => {
  playerCard = (playerCard >= players.length)? 0 // CHECKING TO SEE IF VALUE IS GREATER THAN ARRAY OF PLAYER NUMBERS INPUT than reset by going to first player
    playerCard;
...
setupPlayers = num => {
  ...
  let div = document.createElement('div'); // create div elements for each player (WILL ALWAYS BE 2 or 4)
    ...
    deals.push([]); // multiple arrays within the blank array
      console.log(deals);
        console.log(div);
...
let getDeck = () => {
  ...
  for (let i = 0; i < suit.length; i++) { //iterate as long as i is less than 4
    for (let j = 0; j < cardValues.length; j++) { //iterate as long as j is less than 13
      let card = { //Create objects containing number/letter of card, symbol/suit and ranking.
        cardValue: cardValues[j],
        suit: suit[i],
        cardRank: (j + 1)
      };
      deck.push(card); //inserts each object in to the empty array
    }
  }
